    ...
    for line in watch.Watch().stream(kubeApi.list_cluster_custom_object, 'sectigo.com', 'v1', 'sectigok8soperator', watch=True, resource_version=resourceVersionCompleted):
        ...
        if event_type == "ADDED":
            ...
            if secretExists == True:
                ...
                collect_response = collect(enroll_response, sectigo_cert_type, secretName, resourceVersion, cfmpData)
            ...
            print("############################ COLLECT - response - start ")
            collect_response = collect(replace_response, sectigo_cert_type, secretName, resourceVersion, cfmpData)
            print("############################ COLLECT - response - end ")
            update_post_cert_details(collect_response, secretName, resourceVersion, cfmpData)
